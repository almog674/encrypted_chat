...
class Client:
    def __init__(self, IP, PORT):
        self.IP = IP
        self.PORT = PORT
        (self.nickname, self.password) = self.get_nickname_and_password()
        print(f'nickname: {self.nickname} password {self.password}')
        self.des = Three_des()
        self.rsa = RSA()
    ...
    def get_nickname_and_password(self):
        nickname = input('What is your name? ')
        while ('@' in nickname) or ('/' in nickname):
            print('[SYSTEM] you cant use the characters "@" and "/"')
            nickname = input('What is your name?')
        for i in range(100):
    ...
    def recieve(self):
        while True:
            ...
            # print(message)
            if message == 'NICK':
                hashed_password = self.hash_function.hash_password(self.nickname, self.password)
                to_send = f'{self.nickname}/{hashed_password}'
                ...
                self.client.send(message.encode())
            elif '[SERVER_ENCRYPT]' in message:
                seperated_message = message.split(' ')
                info = seperated_message[0]
                cipher_message_content = seperated_message[1::]
                cipher_message_content = ''.join(cipher_message_content)
                ...
                print(message_to_show.replace('[SERVER_ENCRYPT]', ''))
            elif '#' in message:
                seperated_message = message.split(':')
